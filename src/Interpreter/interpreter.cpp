...
void doOneQuery(string query){
	...
	string first_word = "";	
	string remaining = "";
	if(space_pos!=-1||query!=""){ //分离keyword
...
void createTableClause(string query){
	...
	if(left_bracket_pos==-1){
		...
		string table_name = query.substr(0,left_bracket_pos); //获取table名称
		vector<Attr> attrs;
		...
		// query = "create table table_name(create_definition);"
		...
		if(right_bracket_pos+1>=query.length()){
			...
			Attr lst_attr;
			try{
				lst_attr = parseAttribute(create_definition,is_primarykey_defined,primarykey_column);
				attrs.push_back(lst_attr);
			}catch(int msg){
				if(msg==-1)
					return ;
			}
...
void insertClause(string query){
	...
	// query = "insert into tablename values(content);"
	...
	//insert
	Record record(table.attrs, table.attr_num, values);
	int msg = insertRecord(table, record);
	if (msg == -1) {
